...
namespace Igrice
{
    class Bard : Player
    {
        ...
        public Bard(string name, int level) : base(level)
        {
            this.name = name;
            this.hp += level * 50;
            this.spells.Add("Cosmic binding");
        }
        ...
        public override void CastSpell(string spellNumber, Player opponent)
        {
            switch (spellNumber)
            {
                case "1":
                    ...
                    break;
                case "2":
                    CosmicBinding(opponent);
                    break;
            }
        }

        private void CosmicBinding(Player opponent)
        {
            opponent.takeDmg(400);
        }
    }
}
